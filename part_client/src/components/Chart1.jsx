...
function Chart1() {
  ...
  const filteredData = filterByDateRange(chartData, data1, data2);
  ...
  const groupedData = useMemo(() => {
    return _.groupBy(filteredData, (entry) => {
      ...
      if (ckValue < user.min) {
        return "below_4";
      } else if (ckValue >= user.min && ckValue <= user.max) {
  ...
  const below4Count = groupedData.below_4 ? groupedData.below_4.length : 0;
